//////////////////////////////////////
//  HYPER DREAMS  |  ARTIST SLIDER  //
//////////////////////////////////////
// ALL RIGHTS RESERVED. NAUGHTYDUK© //
//////////////////////////////////////
...
(function () {
  window.rgbKineticSlider = function (options) {
    ...
    // Helper: wait for slide media (videos) to be ready before init
    function waitForSlideMediaReady() {
      const sources = Array.isArray(options.slideImages)
        ? options.slideImages
        : [];
      const readinessPromises = sources.map(function (source) {
        // HTMLVideoElement handling
        if (
          source &&
          source.tagName &&
          source.tagName.toLowerCase() === "video"
        ) {
          const video = source;
        ...
        // Non-video sources (string URLs or IMG/CANVAS elements) – let PIXI load them lazily
        return Promise.resolve();
      });

      return Promise.all(readinessPromises);
    }

    // Load google fonts before starting, then wait for media and init
    window.WebFontConfig = {
      ...
      active: function () {
        waitForSlideMediaReady().then(function () {
